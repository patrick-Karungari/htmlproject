...
class Invest extends \App\Controllers\UserController
{
    ...
    public function create()
    {
        if ($this->request->getPost()) {
            ...
            if (!$plan) return redirect()->back()->with('error', "Investment plan not found");

            if (!is_numeric($amount) || $amount < get_option('minimum_investment', 0)) return redirect()->back()->with('error', "Investment amount is below the minimum");
